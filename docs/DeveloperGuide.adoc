...
This section focuses on the design of TeachConnect's major components.
...
=== Schedule Management
==== Current Implementation
There are two types of schedule: an `Appointment` or a `Task`.
The model diagrams for `Appointment` and `Task` shown in `Figure 24` and `Figure 25`.
...
Similar to `UniquePersonList` and `UniqueTagList`, `UniqueAppointmentList` and `UniqueTaskList` is linked
...
image::chrome_save_as_pdf.png[width="300"]
    Figure 26: Saving document as pdf window in Chrome
