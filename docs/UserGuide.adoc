...
[[UserInterface]]
== User Interface
TeachConnect GUI consists of 5 major components: menu bar, command box, command result box, active list, and calendar view. `Figure 2` annotates the position of each component in the GUI.


image::GUIcomponents.jpg[width="800"]
    Figure 2: Position of GUI components

* Menu bar: +
Contains drop down option for closing TeachConnect or opening the User Guide.
...
* Active List: +
Displays the full list of `contacts`, `students`, `appointments`, `tasks`, `classes` or `shortcuts` stored by TeachConnect, with the active item list selected by the last `list` command. The default list selected upon successful start up of TeachConnect is `contacts`. You can refer to <<List>> for more details. +
Item in the list is shown as a card with an index which can be used for `delete` or `edit` command, together with their details. `Figure 3` shows the components of typical cards in the contact list. A student marker on the top right corner of the card is used to distinguish students from other people.

image::itemCard.jpg[width="800"]
    Figure 3: Components of a card in the Contact List

* Calendar view: +
 Shows all appointment you have within the day, week or month depending on which view mode is selected. You can refer to <<SwitchingCalendarViewmode>> for the different Calendar view mode.
...
Lists all student.
+
image::studentList.jpg[width="800"]
...
+
image::shortcutList.jpg[width="800"]
